...
if os.path.isfile('train.npy'):
    ...
    validate_targets = np.load('validate_targets.npy')
else:
...
CNN_l = 'filters(32,32),kernel_size(5,5)'
Pool_l ='2,2'
...
shuffle_and_save(train,train_targets,'train')
shuffle_and_save(test,test_targets,'test')
shuffle_and_save(validate,validate_targets,'validate')
...
with tf.device('/cpu:0'):
    ...
    with graph.as_default():
        ...
        input_layer = tf.reshape(inputs,[-1,20,tf.shape(inputs)[2],1])
        ...
        kernel_size = [1,1,1,1]
        pool_factor = 2
        number_of_filters = tf.Variable(tf.random_normal([filter_size,filter_size,1,no_of_filters]))

        conv11 = tf.nn.conv2d(input_layer,number_of_filters,kernel_size,padding="SAME")
        #conv12 = tf.layers.conv2d(conv11,number_of_filters,kernel_size,padding="SAME")                                                        
        relu1 = tf.nn.relu(conv11);
        #pdb.set_trace()                                                                                                                       
        number_of_filters = tf.Variable(tf.random_normal([filter_size,filter_size,no_of_filters,no_of_filters]))
        conv12 = tf.nn.conv2d(relu1,number_of_filters,kernel_size,padding="SAME")

        r1 = conv11 + conv12;

        res1 = tf.nn.relu(r1);

        pool1 = tf.layers.max_pooling2d(res1,[2,2],2)

        #pdb.set_trace()                                                                                                                       
        conv21 = tf.nn.conv2d(pool1,number_of_filters,kernel_size,padding="SAME")
        #conv22 = tf.layers.conv2d(conv21,number_of_filters,kernel_size,padding="SAME")                                                        
        relu2 = tf.nn.relu(conv21);

        conv22 = tf.nn.conv2d(relu2,number_of_filters,kernel_size,padding="SAME")

        r2 = conv21 + conv22;
        
        res2 = tf.nn.relu(r2);

        pool2 = tf.layers.max_pooling2d(res2,[2,2],2)

        
        # e.g: log filter bank or MFCC features                                                                                                
        # Has size [batch_size, max_stepsize, num_features], but the                                                                           
        # batch_size and max_stepsize can vary along each step                                                                                 
        pool2_flat = tf.reshape(pool2, [-1, tf.shape(pool2)[2],32*5]) 
       
        inputs_rnn = pool2_flat       
    ...
    with tf.Session(graph=graph) as session:
        ...
        for curr_epoch in range(s,num_epochs):
            ...
            print " Validation Error :",session.run(ler,feed_dict = {inputs:validation_input,targets:validation_target,seq_len:validation_len})
            ...
            if (curr_epoch)%10 == 0:
                ...
                print " Test Error :",session.run(ler,feed_dict = feed)
